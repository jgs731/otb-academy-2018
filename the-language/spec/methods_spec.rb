...
RSpec.describe "methods in ruby" do
  ...
  it "is not possible to call methods with the wrong number of arguments" do
    expect { a_global_method }.to raise_error( ArgumentError, /given 0, expected 2/ ) # HINT: Fill in the error and part of the message

    expect { a_global_method(1,2,3) }.to raise_error( ArgumentError, /given 3, expected 2/ ) # HINT: Fill in the error and part of the message
  end
  ...
  context "methods on objects" do
    ...
    it "raises an error if a private method has an explicit receiver" do
      expect { an_object.calls_the_private_method_on_self }.to raise_error( NoMethodError, // )
    end
